...
Utils.GetModePatternFromDiatonicScale = function(diatonicScale_) {
    ...
    while (result.length < 7) {
        stepValue += 1;
        if (tempChromaticScale.peek().has(tempDiatonicScale.peek(), function(left, right) {
...
Utils.AttachPlayNoteToDiatonicScale = function(diatonicScale, playLengthSeconds) {
    var transitionStartTime = 0.08;
    var transitionEndTime = 0.08;
    var gainValue = 0.2;
    ...
    diatonicScale.forEach(function(note) {
        note.playNote = function(curNote, audioCtx) {
            ...
            gainNode.gain.linearRampToValueAtTime(gainValue, audioCtx.currentTime + transitionStartTime);
            ...
            gainNode.gain.setValueAtTime(gainValue, audioCtx.currentTime + playLengthSeconds + transitionStartTime);
            gainNode.gain.linearRampToValueAtTime(0, audioCtx.currentTime + playLengthSeconds + transitionStartTime + transitionEndTime);
            oscillator.stop(audioCtx.currentTime + playLengthSeconds + transitionStartTime + transitionEndTime);

            setTimeout(function(curNote_, gainNode_, audioCtx_, oscillator_) {
                $('.diatonic-scale').children('li').each(function() {
                    if ($(this).text().trim() === curNote.Name) {
                        $(this).removeClass('active');
                    }
                });
            }, (playLengthSeconds * 1000) + (transitionStartTime * 1000), curNote, gainNode, audioCtx, oscillator);
        };
    });
}
